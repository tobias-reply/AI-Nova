...
def create_documentation_pr(doc_branch_info, changed_files):
    ...
    try:
        ...
        if result.returncode != 0:
            print("❌ GitHub CLI (gh) not found. Using GitHub API fallback...")
            return create_pr_with_api(doc_branch_info, changed_files)
...
- [ ] TODO comments are reasonable"""
        ...
        print("Trying GitHub API fallback...")
        create_pr_with_api(doc_branch_info, changed_files)

def create_pr_with_api(doc_branch_info, changed_files):
    """Create PR using GitHub API when gh CLI is not available"""
    try:
        import json
        ...
        if not github_token:
            print("❌ GITHUB_TOKEN not found. Cannot create PR via API.")
            print(f"You can manually create a PR for branch: {doc_branch_info['branch_name']}")
            return
...
- [ ] TODO comments are reasonable"""
        ...
        req = urllib.request.Request(url, data=json.dumps(pr_data).encode(), headers=headers, method='POST')
