...
class DocumentationAgent:
    ...
    def __init__(self, max_turns: int = 5, model: str = None):
        self.max_turns = max_turns
        # Use simple, reliable model for Bedrock or default for API
        self.model = model or self._get_default_model()
        self._check_configuration()
        self.system_prompt = """You are a documentation assistant with access to development tools.
...
- Focus ONLY on: comments, docstrings, type hints, and documentation files

Your tasks:
1. Add or improve function/class docstrings following standard conventions
2. Add inline comments for complex logic or non-obvious code
3. Add type hints where missing
4. Update or create documentation files if needed
5. If you identify code issues, add #TODO comments with descriptions

Always examine files thoroughly and improve documentation without modifying functional code."""
    ...
    async def process_changed_files(self, changed_files: List[str], project_root: str = ".") -> Dict[str, Any]:
        """Process a list of changed files for documentation improvements"""
        if not changed_files:
            print("No changed files to process.")
            return {"status": "no_files", "files_processed": 0}

        # Create the prompt with changed files list
        files_text = "\n".join([f"- {file}" for file in changed_files])
        prompt = f"""I have the following files that were recently changed and need documentation review:
...
Start by examining the first file and work through each one systematically."""
    ...
    async def general_documentation_review(self, project_root: str = ".") -> Dict[str, Any]:
